...
function buildTurnOrder() {
  const units = [
    'firstPosEnemy', 'secondPosEnemy', 'thirdPosEnemy',
    'firstPos', 'secondPos', 'thirdPos'
  ]
    .map(id => battleUnits[id])
    .filter(u => u && u.hp > 0);

  battleState.turnOrder = shuffleArray(units);
  battleState.turnIndex = 0;
}
...
function nextTurn() {
  ...
  while (safety < 20) {
    ...
    if (unit.side === 'enemy') {
      ...
      infoText.textContent = `${unit.id} ходит`;
      clearButtons();
      addButton('Просто нажми на цель', startPlayerAttack);
    }
...
function resetBattleState() {
  ...
  battleState.waitingForTarget = false;
  battleState.activeUnit = null;
  battleState.selectedAbility = null;
...
function resetEnemyHp() {
  battleUnits.firstPosEnemy.hp = battleUnits.firstPosEnemy.maxHp;
  battleUnits.secondPosEnemy.hp = battleUnits.secondPosEnemy.maxHp;
  battleUnits.thirdPosEnemy.hp = battleUnits.thirdPosEnemy.maxHp;
}
function resetAllyHp() {
  battleUnits.firstPos.hp = battleUnits.firstPos.maxHp;
  battleUnits.secondPos.hp = battleUnits.secondPos.maxHp;
  battleUnits.thirdPos.hp = battleUnits.thirdPos.maxHp;
}
